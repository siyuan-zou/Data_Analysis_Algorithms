...
int main(int argc, char* argv[]) {
	...
	if ((projection_dim < 1) | (projection_dim >= train_dataset.get_dim() - 1)) { 
    ...
    for (int i = 0; i < class_dataset.get_n_samples(); i++) {
        ...
        annDeallocPt(query);
        confusion_matrix_original.add_prediction(true_label, predicted_label);
    }
    ...
    std::cout << endl
         ...
         << "ms\n\n";
    confusion_matrix_original.print_evaluation();
    ...
    clock_t t_knn_test_projected = clock();
        for (int i = 0; i < projection_test_dataset.get_n_samples(); i++) {
        std::vector<double> sample = projection_test_dataset.get_instance(i);
        ANNpoint query = annAllocPt(projection_test_dataset.get_dim() - 1, 0);
        int true_label = -1;  // To not leave it uninitialized + will error in AddPrediction
        for (int j = 0, j2 = 0; j < projection_test_dataset.get_dim() && j2 < projection_test_dataset.get_dim(); j++, j2++) {
        ...
        annDeallocPt(query);
        confusion_matrix_projected.add_prediction(true_label, predicted_label);
    }
    ...
    std::cout << endl
         ...
         << "ms\n\n";
    confusion_matrix_projected.print_evaluation();
