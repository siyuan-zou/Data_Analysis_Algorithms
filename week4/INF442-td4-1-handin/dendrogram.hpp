...
protected:
    ...
    // Union–find structure elements (Exercise 4)
    int *parent;
    int *rank;

    // Binary-tree structure elements (Exercise 4)
    int *left;
    int *down;
    ...
    int nonsingle_clusters; // Number of non-singular clusters

    // List of significant heights (Exercise 5)
    double *sign_heights = nullptr;
    int count_sign_heights;

    void merge(edge *e);
    int _count_ns_clusters(); // Counts the non-singular clusters from scratch
