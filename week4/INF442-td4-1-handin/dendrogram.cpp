...
void dendrogram::merge(edge *e) {
    ...
    // 4. Update ranks
    // 5. Update heights
    int p1_rep = find(e->get_p1());
    int p2_rep = find(e->get_p2());
    int mom; int child;


    // merge and assign parents
    if (p1_rep == p2_rep) {
        return;
    }else { // they are not in the same cluster
        if (rank[p1_rep] == rank[p2_rep]) {

            if (p1_rep > p2_rep) {
                mom = p1_rep; child = p2_rep;
            }else if (p1_rep < p2_rep) {
                mom = p2_rep; child = p1_rep;
            } // take the large index to be the parent

            // update rank
            rank[mom] += 1;

        } else if (rank[p1_rep] > rank[p2_rep]) {
            mom = p1_rep; child = p2_rep;
        } else if (rank[p1_rep] < rank[p2_rep]) {
            mom = p2_rep; child = p1_rep;
        } // take the large rank to be the parent
...
int dendrogram::_count_ns_clusters() {
    ...
    for (int i=0; i < g->get_num_nodes(); i++) {
        ...
        for (int j=0; j < g->get_num_nodes(); j++) {
            if (clusters[j] == i) {num_points += 1;}
        }

        if (num_points == 1) {count += 1;}
    }

    return total_clusters - count;
}
...
double dendrogram::get_cluster_height(int cluster) {
    ...
    int num_points = 0;
    for (int i=0; i<g->get_num_nodes(); i++) {
        if (clusters[i] == cluster) {num_points += 1;}
    }

    if (num_points == 1) {return 0;}
    else {
        int child = down[cluster];

        if (clusters[child] == cluster) {return height[child];}

        while (clusters[child] != cluster) {
            if (left[child] != -1) {
                child = left[child];
            }else {
                child = down[child];
