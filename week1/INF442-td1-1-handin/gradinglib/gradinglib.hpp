...
//    s << "std::vector<" << tname << ">{";
  ...
  template <typename T1, typename T2, typename... Arguments>
  bool test_eq_array_ptr(std::ostream &out,
                   const std::string &function_name,
                   T1* result_start,
                   T2* expected_start,
                   int length,
    ...
    bool success = true;
    for (int i = 0; i < length; ++i) {
        success = success && (result_start[i] == expected_start[i]);
    }
    out << (success ? SUCCESS : FAILURE);
    ...
    out << ": got ";
    print_array(out, result_start, length);
    out << " expected ";
    print_array(out, expected_start, length);
    out << std::endl;
  ...
  template <typename T1, typename T2, typename... Arguments>
  bool test_eq_array_ptr_approx(std::ostream &out,
                   const std::string &function_name,
                   T1* result_start,
                   T2* expected_start,
                   int length,
    ...
    bool success = true;
    for (int i = 0; i < length; ++i) {
        success = success && (result_start[i] - expected_start[i] > -eps) && (result_start[i] - expected_start[i] < eps);
    }
    out << (success ? SUCCESS : FAILURE);
